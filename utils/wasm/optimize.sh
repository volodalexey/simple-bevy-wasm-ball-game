...
# optimize prepared wasm build
mkdir -p target/wasm32-unknown-unknown/optimized
wasm-opt -Oz --output target/wasm32-unknown-unknown/optimized/simple-bevy-wasm-ball-game_bg.wasm target/wasm32-unknown-unknown/bindgen/simple-bevy-wasm-ball-game_bg.wasm

mkdir -p dist
# remove all files in dist folder except .git folder
find dist -mindepth 1 -not \( -path '*/.git' \) -not \( -path '*/.git/*' \) -exec rm -rf {} +
cp -r assets dist/assets
cp target/wasm32-unknown-unknown/optimized/simple-bevy-wasm-ball-game_bg.wasm dist/
